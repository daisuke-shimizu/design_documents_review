...
- 最終的に何をどれくらい購入されたかという情報はどこに保存されるようにしましょう。その情報が購入履歴になります。
   - [attention]この文章だと、受講生が混乱すると思います。

   1.まずは問題となっている部分を明確にしましょう。→「何をどれくらい購入したという情報は、どこに保持されますか。」
   2.やるべきことの大まかな内容を伝えましょう。→「エンティティを分割するなど、テーブルの構造を見直してみましょう。」
   3.ここから先は、直接口頭で説明しに行っても良いかもしれません。
...
## エンドユーザー
- [add]PKの名前がユーザーIDとなっており、テーブル名(エンドユーザー)と異なりますので、統一する必要があります。
...
　* 運営上、「いつ、どの商品が入荷したか」という情報を管理しておく方が安全な運用です。
 ...
 - トラックNo.カラムとありますが、第三者から見て分かりやすいカラムの命名をお願いします。
   - [attention]トラックNo.はどのエンティティに対して紐づく情報化か、考えてみましょう。
     ディスクごとに「1,2,3...」与えられるのではなく、曲に対して「1,2,3...」と与えられるものかと思います。
 ...
 - 「購入テーブル」という名前ですが、テーブルの命名はエンドユーザー視点でお願いします。データベースの情報は管理者が利用するためです。
   -  [add]購入テーブルと購入詳細テーブルについて、リレーションが不適切ですので、追加で指摘が必要です。
 
 ## 購入詳細テーブル
 - すでに触れた指摘ですが、「購入詳細テーブル」という命名を再考してください。命名はエンドユーザー視点でお願いします。
   - [attention]指摘事項はOKですが、この場合「命名は管理者視点」となります。
...
[add]
## ジャンル・レーベル
- これらのテーブルの「ジャンル」「レーベル」というカラム名は、ジャンルについてのどういう属性を示しているか分かりませんので、「ジャンル名」「レーベル名」などとするのが適切です。
